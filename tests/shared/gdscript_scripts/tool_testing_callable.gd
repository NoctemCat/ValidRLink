...
@export var hidden_action := test_hidden
@export var stop_action := test_disabled
@export var undoredo_action := test_undoredo

@export var make_green_action := compat_unbind(set_self_modulate.bind(Color.GREEN))
@export var clear_modulation_action := compat_unbind(set_self_modulate.bind(Color.WHITE))
...
func _get(property: StringName) -> Variant:
    if property.begins_with("cool_dynamic_tool_button_"):
        var callable := test_dynamic.bind(property.trim_prefix("cool_dynamic_tool_button_"))
        return compat_unbind(callable)
    return null

func compat_unbind(callable: Callable) -> Callable:
    if Engine.get_version_info()["hex"] < 0x040300: 
        # `get_argument_count` doesn't exist, so the  
        # helper will always be passed, ignore it
